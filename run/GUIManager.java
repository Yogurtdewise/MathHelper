 ...
 *  back-end logic. Used to manage which "screen" is to be displayed, passing required parameters to
 *  the appropriate screen. This class is the first class that should be called by the project's Event
 *  Dispatch Thread.
 * NOTE: PreKModuleSelect was never intended for the Grade 1-2 and Grade 3-4 ModuleSelectScreens.
 *  However, no distinct design changes were ever made for Grades 1-2 and 3-4. Therefore, for simplicity,
 *  PreKModuleSelect is re-used for  Grades 1-2 and 3-4.
...
public final class GUIManager{
	 ...
	 *  gm2 - Constructor                *
	 *  gm3 - Initialize & Run           *
	 *  gm4 - Directory Structure        *
	 *  gm5 - Database Management        *
	 *  gm6 - Window Management          *
	 *  gm7 - Exception Handling         *
	 *  gm8 - Shutdown                   *
	 *  gm9 - TODO TEST METHODS          *
	 *                                   *
	 *************************************/
 ...
 *************************/
	...
	private static final String DB_FILEPATH = System.getProperty("user.dir") + "\\db.dat";
	
	private static GUIManager singleInstance = new GUIManager(); //The singleton instance of GUIManager.
	
	//Exception handler variables.
	private String runTimeErrorSeparator = "**** NEW LOG ****"; //Used to indicate a new error log in a single error log file.
	private StringBuffer errorLog = new StringBuffer(runTimeErrorSeparator + "\r\n");
	private int errorNumber = 1;  //A counter for each caught exception during this run.
 ...
 **********************/
	...
	/**
	 * The private constructor of GUIManager. Sets the exception handler for this thread, and adds
	 *  FontMaker fonts to this graphics environment. Additionally, initializes the database if none
	 *  exists, or reads a pre-existing database.
	 */
	private GUIManager(){
 ...
 ***************************/
	...
	private void initStudent(){
		studentFolderName = database.getLastName(userName) + ", " + database.getFirstName(userName);
 ...
 ******************************/
	...
	/**
	 * Used to ensure the student's directory structure is ready to be written to.
	 * TODO The "Tests" folder might iterate through all of a student's possible test folders.
	 */
	...
	/**
	 * A convenience method used to obtain the path, from the program's directory, to the student's
	 *  "Tests" folder.
	 * @return A String describing the path, from the program's directory, to the student's "Tests" folder.
	 */
	 ...
	 * A convenience method used to obtain the path, from the program's directory, to the student's
	 *  "Rewards" folder.
	 * @return A String describing the path, from the program's directory, to the student's "Rewards" folder.
	 */
 ...
 ******************************/
	...
	/**
	 * Used to obtain the String password of a specified username. Returns null if the
	...
	/**
	 * Used to obtain the current user's grade for the specified test and difficulty level.
	 * @param button A ModuleSelectButtonInterface that describes the test who's grade is to be retrieved.
	 * @param difficulty A DifficultyLevel that describes the difficulty level of the test who's grade
	 *  is to be retrieved.
	 * @return An int describing the current user's number of correct answers for the specified test
	 *  and difficulty level.
	 */
	public int getPreviousGrade(ModuleSelectButtonInterface button, DifficultyLevel difficulty){
		return ReportCard.getGrade(database.getReportCard(userName), button, difficulty);
	}
	
	/**
	 * Used to set the current users grade and maximum attainable grade for the specified test and
	 *  difficulty level.
	 * @param button A ModuleSelectButtonInterface that describes the test who's grade is to be changed.
	 * @param difficulty A DifficultyLevel that describes the difficulty level of the test who's grade
	 *  is to be changed.
	 * @param numCorrect An int describing the number of correct answers that the current user achieved for the
	 *	specified test and difficulty level. The number will be recorded to the user's database record for the
	 *  specified test and difficulty level.
	 * @param maxGrade An int describing the maximum attainable grade for the specified test and difficulty level,
	 *  which will also be set for the specified test and difficulty level.
	 */
	public void setGrade(ModuleSelectButtonInterface button, DifficultyLevel difficulty, int numCorrect, int maxGrade){
		HashMap<ModuleSelectButtonInterface, ModuleReportSummary> reportCard = database.getReportCard(userName);
		database.setReportCard(userName, ReportCard.setGrade(reportCard, button, numCorrect, maxGrade, difficulty));
	}
 ...
 ****************************/
	...
	/**
	 * Used to obtain the MainWindow that is used as a primary container for all displayed screens.
	 * @return The MainWindow that is used as a primary container for displayed objects.
	 */
	...
	/**
	 * Creates a new WelcomeScreen that will change mainWindow's background and populate mainWindow
	 *  with appropriate JComponents. All Exceptions are thrown to the caller method.
	 * @throws IOException {@link project.screens.WelcomeScreen See the WelcomeScreen's constructor details}.
	 */
	...
	/**
	 * Creates a new ReportCardScreen of the specified type, using the specified reportMap.
	 * TODO This method must be changed to get the databases' HashMap!
	 * @throws IOException {@link project.screens.ReportCardScreen See the ReportCardScreen's constructor details.}
	 */
	public void buildReportCardScreen() throws IOException{
		int size;
		HashMap<ModuleSelectButtonInterface, ModuleReportSummary> reportMap = database.getReportCard(userName);
		switch(gradeLevel){
			case 0:  size = PreKModuleSelectTestButtons.Button.values().length;
					 new ReportCardScreen(this, new PreKModuleSelectTestButtons(size), reportMap);
					 break;
			case 1:  size = Grade1ModuleSelectTestButtons.Button.values().length;
					 new ReportCardScreen(this, new Grade1ModuleSelectTestButtons(size), reportMap);
					 break;
			case 2:  size = Grade3ModuleSelectTestButtons.Button.values().length;
					 new ReportCardScreen(this, new Grade3ModuleSelectTestButtons(size), reportMap);
					 break;
			default: size = PreKModuleSelectTestButtons.Button.values().length;
			 		 new ReportCardScreen(this, new PreKModuleSelectTestButtons(size), reportMap);
			 		 break;
		}
	}
	...
	/**
	 * Creates a new ModuleSelect screen for the current gradeLevel when a user selects
	 *  the "Test" option. Invoking this method will change mainWindow's background and
	 *  populate mainWindow with appropriate JComponents. All Exceptions are thrown to
	 *  the caller method.
	 * @throws IOException {@link project.screens.PreKModuleSelect See the PreKModuleSelect constructor details.}
	 */
	public void buildTestModuleSelect() throws IOException{
		int lastActiveButton = database.getLastActiveTest(userName);
		switch(gradeLevel){
			case 0:  new PreKModuleSelect(this, new PreKModuleSelectTestButtons(lastActiveButton)) ;  break;
			case 1:  new PreKModuleSelect(this, new Grade1ModuleSelectTestButtons(lastActiveButton)); break;
			case 2:  new PreKModuleSelect(this, new Grade3ModuleSelectTestButtons(lastActiveButton)); break;
			default: new PreKModuleSelect(this, new PreKModuleSelectTestButtons(lastActiveButton))  ; break;
		}
	}
	
	/**
	 * Creates a new ModuleSelect screen for the current gradeLevel when a user selects the
	 *  "Practice" option. Invoking this method will change mainWindow's background and populate
	 *  mainWindow with appropriate JComponents. All Exceptions are thrown to the caller method.
	 * @throws IOException {@link project.screens.PreKModuleSelect See the PreKModuleSelect constructor details.}
	 */
	public void buildPracticeModuleSelect() throws IOException{
		switch(gradeLevel){
			case 0:  new PreKModuleSelect(this, new PreKModuleSelectPracticeButtons()) ;  break;
			case 1:  new PreKModuleSelect(this, new Grade1ModuleSelectPracticeButtons()); break;
			case 2:  new PreKModuleSelect(this, new Grade3ModuleSelectPracticeButtons()); break;
			default: new PreKModuleSelect(this, new PreKModuleSelectPracticeButtons())  ; break;
		}
	}
	
	/**
	 * Creates a new ModuleSelect screen for the current gradeLevel when a user selects
	 *  the "Tutorial" option. Invoking this method will change mainWindow's background
	 *  and populate mainWindow with appropriate JComponents. All Exceptions are thrown
	 *  to the caller method.
	 * @throws IOException {@link project.screens.PreKModuleSelect See the PreKModuleSelect constructor details.}
	 */
	public void buildTutorialModuleSelect() throws IOException{
		switch(gradeLevel){
			case 0:  new PreKModuleSelect(this, new PreKModuleSelectTutorialButtons()) ;  break;
			case 1:  new PreKModuleSelect(this, new Grade1ModuleSelectTutorialButtons()); break;
			case 2:  new PreKModuleSelect(this, new Grade3ModuleSelectTutorialButtons()); break;
			default: new PreKModuleSelect(this, new PreKModuleSelectTutorialButtons())  ; break;
		}
	}
 ...
 *****************************/
	...
	/**
	 * This method is used as a catch-all for all Exceptions thrown by this thread. It aids in
	 *  error reporting by creating an error log, and provides user feedback by using an pop-up
	 *  error dialogue before the thread dies.
 ...
 *******************/
	...
	/**
	 * Used to gracefully exit the program. Creates an error log text file, if there are errors to
	 *  report.
	 */
	public void exit(){
		if(errorNumber > 1){
			...
			String fileName = date.get(Calendar.YEAR) + "_" + date.get(Calendar.DAY_OF_MONTH) + "_" 
					+ date.get(Calendar.HOUR_OF_DAY) + "_" + date.get(Calendar.MINUTE);
			try {
				TextFileMaker.writeString("ErrorLogs\\", fileName, errorLog.toString());
			} catch (IOException e) {
